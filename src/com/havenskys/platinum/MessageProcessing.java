    	...
    	if( keySendList.moveToFirst() ){
	    	...
	    	if(messageId > 0){
				...
				for( int i = 0; i < messageDivided.size(); i++){
					sentIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_SENT_CONFIRMED", messageUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("messageid", messageId).putExtra("totalcnt", messageDivided.size() ), 0));
				}
		    	
		    	ArrayList<PendingIntent> deliveryIntents = new ArrayList<PendingIntent>(messageDivided.size());
		    	for( int i = 0; i < messageDivided.size(); i++){
		    		deliveryIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_DELIVERY_CONFIRMED", messageUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("messageid", messageId).putExtra("totalcnt", messageDivided.size() ), 0)); //sent
		    	}
	...
	*/		
    ...
    //Log.w(TAG,"Writing decrypted to log " + decrypted);
       ...
       //Log.w(TAG,"sendRSA("+recipientId+") recipientStore UPDATE uri to outgoingUri("+outgoingUri.toString()+")");
		...
		ArrayList<PendingIntent> sentIntents = new ArrayList<PendingIntent>(messageDivided.size());
		for( int i = 0; i < messageDivided.size(); i++){
			sentIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_SENT_CONFIRMED", outgoingUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("recipientid", recipientId).putExtra("setstatus", 7).putExtra("totalcnt", messageDivided.size() ), 0));
		}
    	
    	ArrayList<PendingIntent> deliveryIntents = new ArrayList<PendingIntent>(messageDivided.size());
    	for( int i = 0; i < messageDivided.size(); i++){
    		deliveryIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_DELIVERY_CONFIRMED", outgoingUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("recipientid", recipientId).putExtra("setstatus", 4).putExtra("totalcnt", messageDivided.size() ), 0)); //sent
    	}
    ...
    //Log.i(TAG,"displaySurfaceDialog()");
    	Intent smsDialogIntent  = new Intent(context, com.havenskys.platinum.activity.SurfaceDialog.class)
			...
			for( int recipientPosition = 0; recipientPosition < recipientRow.getCount(); recipientPosition++ ){
				...
				if( rUseRSA ){
		          ...
		          //smsMan.sendMultipartTextMessage(recipientTel, null, messageDivided, null, null);
		            ...
		            sentIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_SENT_CONFIRMED", outgoingUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("recipientid", recipientId).putExtra("setstatus", 11).putExtra("totalcnt", messageDivided.size() ), 0));
		        	
		        	ArrayList<PendingIntent> deliveryIntents = new ArrayList<PendingIntent>(1);
		            deliveryIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_DELIVERY_CONFIRMED", outgoingUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("recipientid", recipientId).putExtra("setstatus", 3).putExtra("totalcnt", messageDivided.size() ), 0)); //sent
		           ...
		           //Log.w(TAG,"Iterating recipientStore " + recipientPosition + " recipientStore UPDATE uri to outgoingUri("+outgoingUri.toString()+")");
		    		...
		    		ArrayList<PendingIntent> sentIntents = new ArrayList<PendingIntent>(1);
			        sentIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_SENT_CONFIRMED", outgoingUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("recipientid", recipientId).putExtra("totalcnt", messageDivided.size() ) , 0));
			    	
			    	ArrayList<PendingIntent> deliveryIntents = new ArrayList<PendingIntent>(1);
			        deliveryIntents.add(PendingIntent.getBroadcast( mContext, 0, new Intent("com.rsasms.MessageReceiver.MESSAGE_DELIVERY_CONFIRMED", outgoingUri, mContext, com.havenskys.platinum.MessageReceiver.class).putExtra("recipientid", recipientId).putExtra("totalcnt", messageDivided.size() ) , 0));
